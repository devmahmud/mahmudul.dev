...
description: "Learn why using GET inside Next.js Server Actions creates unnecessary round trips, how to fetch data efficiently with React Server Components, and when to use Server Actions the right way — with fun analogies and diagrams."
...
If you're diving into **Next.js App Router** and experimenting with **Server Actions**, you might be tempted to wrap every fetch in them — including `GET` requests.

But here's the catch: **GET inside Server Actions is like calling your mom to ask if you can text her.** Totally unnecessary. 🙃
...
👉 **Why POST?** Server Actions use POST requests for security and consistency. Unlike GET requests (which can be cached, bookmarked, or logged), POST requests ensure mutations are intentional and can't be accidentally triggered by web crawlers or browser prefetching.
...
💥 Problem: You just created **two round trips**.

1. Client → Server Action (`POST` request to run `getTodos`)
2. Server Action → API (`GET` request for todos)

This is slower than if you had just fetched the data directly in a **React Server Component**.

---

## 🔍 Visualizing the Double Round Trip
...
<svg width="500" height="300" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect x="50" y="20" width="100" height="40" rx="5" fill="#f0f0f0" stroke="#000"/>
  <text x="100" y="45" text-anchor="middle">Client</text>
  
  <rect x="200" y="20" width="100" height="40" rx="5" fill="#f0f0f0" stroke="#000"/>
  <text x="250" y="45" text-anchor="middle">ServerAction</text>
  
  <rect x="350" y="20" width="100" height="40" rx="5" fill="#f0f0f0" stroke="#000"/>
  <text x="400" y="45" text-anchor="middle">API</text>

  
  <line x1="100" y1="60" x2="100" y2="280" stroke="#000" stroke-dasharray="5,5"/>
  <line x1="250" y1="60" x2="250" y2="280" stroke="#000" stroke-dasharray="5,5"/>
  <line x1="400" y1="60" x2="400" y2="280" stroke="#000" stroke-dasharray="5,5"/>

 
  <line x1="100" y1="100" x2="250" y2="100" stroke="#000" marker-end="url(#arrowhead)"/>
  <text x="175" y="95" text-anchor="middle">POST (run getTodos)</text>

  <line x1="250" y1="150" x2="400" y2="150" stroke="#000" marker-end="url(#arrowhead)"/>
  <text x="325" y="145" text-anchor="middle">GET /todos</text>

  <line x1="400" y1="200" x2="250" y2="200" stroke="#000" stroke-dasharray="5,5" marker-end="url(#arrowhead)"/>
  <text x="325" y="195" text-anchor="middle">todos JSON</text>

  <line x1="250" y1="250" x2="100" y2="250" stroke="#000" stroke-dasharray="5,5" marker-end="url(#arrowhead)"/>
  <text x="175" y="245" text-anchor="middle">todos JSON</text>
...
⚡ Notice how the client goes through an extra **middleman**. That's wasted time.
...
|----------|-------------|--------------|---------|
| **Server Action + GET** | 2 | ~400-800ms | ❌ No caching |
| **React Server Component** | 1 | ~200-400ms | ✅ Built-in caching |

**Real-world example:**
- Server Action approach: 600ms (300ms client→server + 300ms server→API)
- RSC approach: 250ms (direct server→API at render time)

That's a **58% performance improvement** just by using the right pattern! 🚀
...
⚡ This happens **at render time** on the server, streamed to the client. Super efficient.

**Key benefits:**
- ✅ **Built-in caching** with Next.js fetch extensions
- ✅ **Automatic revalidation** keeps data fresh
- ✅ **Streaming support** for better perceived performance
- ✅ **Error boundaries** can catch and handle failures gracefully
...
* **Never for pure GETs** (unless you’re doing something weird like combining data after a mutation).
...
That's the sweet spot for Server Actions. 🚀

---

## 🤔 Edge Cases: When GET in Server Actions Might Be OK
...
**⚠️ Rule of thumb**: Only use GET in Server Actions when you need server-side logic that can't be done in RSCs.
...
```tsx
// ❌ Before: Calling Server Action from client
'use client';
...
* **GET → Use React Server Components**
* **POST → Use Server Actions**
* Don’t wrap GETs in Server Actions unless you like extra lag 😅
...
* **GET in RSC** → Walking straight to the fridge to grab cookies.
* **GET in Server Action** → Calling your roommate to ask them to go to the fridge, grab cookies, and bring them to you. Then they call the bakery to bake cookies first. 🙃

One is instant. The other? A comedy of delays.

So next time you're tempted to use `GET` in a Server Action… just walk to the fridge yourself. 🍪✨
